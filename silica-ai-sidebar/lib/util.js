...
export async function getHost() {
  try {
    const o = await chrome.storage.local.get(HOST_KEY);
    if (o && o[HOST_KEY]) return o[HOST_KEY];
  } catch (_) {
...
export async function setHost(value) {
  ...
  if (!v) v = DEFAULT_HOST;
  try { await chrome.storage.local.set({[HOST_KEY]: v}); }
  catch (_) { localStorage.setItem(HOST_KEY, v); }
  return v;
}
